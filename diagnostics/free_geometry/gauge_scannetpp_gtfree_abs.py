#!/usr/bin/env python3
"""GT-free gauge-repair factor search for the C2M_ABS arm @100v (20 scenes).
Copy of gauge_scannetpp_gtfree_scl.py with ARMS=["C2M_ABS"]; f_gt reference from
gauge_scan/gauge_mismatch_C2M_ABS.csv.

Objective A: conf-weighted mean cross-frame nearest-neighbour distance (argmin).
Objective B: conf-weighted inlier ratio at adaptive threshold (argmax).
Grid s in [0.90, 1.10] step 0.005. Writes gauge_scan/gauge_gtfree_C2M_ABS.json +
gauge_gtfree_curves_C2M_ABS.csv. Pure CPU. New file; no existing module
modified."""
...
ARMS = ["C2M_ABS"]
# Wider than the other arms' [0.90,1.10]: ABS mismatch reaches -11.2% ->
# f_gt up to 1.126; capping at 1.10 would truncate 3 scenes' factors.
GRID = np.arange(0.90, 1.15 + 1e-9, 0.005)
PTS_PER_FRAME = 700
K_NN = 12
...
def main():
    ...
    f_gt = {}
    gm_path = os.path.join(OUT, "gauge_mismatch_C2M_ABS.csv")
    ...
    with open(os.path.join(OUT, "gauge_gtfree_C2M_ABS.json"), "w") as f:
        json.dump(results, f, indent=1)
    with open(os.path.join(OUT, "gauge_gtfree_curves_C2M_ABS.csv"), "w", newline="") as f:
