#!/usr/bin/env python3
"""Regenerate eval npz (depth/extr/intr/conf) for the C2M_ABS arm: 20 scannetpp
scenes @100v, from final_protocol ckpts. Copy of gauge_scannetpp_infer_scl.py
with ARMS=["C2M_ABS"] (npz carry conf for the GT-free factor search). Output
goes to <scannetpp>/gauge_scan (nothing under eval32/ is touched). Resumable.
GPU inference; waits until >=15 GB free. New file; does not modify any existing
