#!/usr/bin/env python3
"""Counterfactual gauge-repair fusion for the C2M_ABS arm on scannetpp @100v.
Copy of gauge_scannetpp_fuse_scl.py restricted to arms=["C2M_ABS"], factors from
the ABS-specific files:
  orig    : factor 1.0 (re-fused in the same pass -> strictly paired)
  fixgt   : f_gt = s_depth/s_pose from gauge_scan/gauge_mismatch_C2M_ABS.csv
  fixfree : f_free_dist from gauge_scan/gauge_gtfree_C2M_ABS.json
Variant npz go to gauge_scan/eval32/C2M_ABS_{orig,fixgt,fixfree}@100v; metrics
to gauge_scan/gauge_fix_metrics_C2M_ABS.json. Variant npz extrinsics/intrinsics
asserted bit-identical to source (AUC structural invariance).
...
does not modify any existing module. Run from repo root:
  CUDA_VISIBLE_DEVICES="" python3 diagnostics/free_geometry/gauge_scannetpp_fuse_abs.py \
      --scenes <20 scenes> --variants orig fixgt fixfree
"""
...
def load_factors():
    f_gt, f_free = {}, {}
    gm = os.path.join(OUT, "gauge_mismatch_C2M_ABS.csv")
    if os.path.isfile(gm):
        with open(gm) as f:
            for row in csv.DictReader(f):
                f_gt[(row["arm"], row["scene"])] = \
                    float(row["s_depth"]) / float(row["s_pose"])
    gf = os.path.join(OUT, "gauge_gtfree_C2M_ABS.json")
...
def main():
    ...
    for arm in args.arms:
        for variant in args.variants:
            ...
            with open(os.path.join(OUT, "gauge_fix_metrics_C2M_ABS.json"), "w") as fp:
