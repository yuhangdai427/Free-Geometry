#!/usr/bin/env python3
"""Gauge-mismatch measurement for the C2M_ABS arm over 20 scannetpp scenes @100v.
Copy of gauge_scannetpp_measure_scl.py with ARMS=["C2M_ABS"], paired with the
A0_baseline inside eval32_metrics_C2M_ABS.json.
...
Writes gauge_scan/gauge_mismatch_C2M_ABS.csv + .json and
gauge_correlation_C2M_ABS.json. Pure CPU. New file; no existing module modified."""
...
ARMS = ["C2M_ABS"]
METRIC_FILES = {"C2M_ABS": "eval32_metrics_C2M_ABS.json"}
...
def main():
    ...
    with open(os.path.join(OUT, "gauge_mismatch_C2M_ABS.csv"), "w", newline="") as f:
        ...
        for r in rows:
            w.writerow(r)
    with open(os.path.join(OUT, "gauge_mismatch_C2M_ABS.json"), "w") as f:
    ...
    with open(os.path.join(OUT, "gauge_correlation_C2M_ABS.json"), "w") as f:
