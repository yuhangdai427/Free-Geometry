#!/usr/bin/env python3
"""GT-free gauge-repair factor search for the C2M_SCL arm @100v (20 scenes).
Copy of gauge_scannetpp_gtfree.py restricted to ARMS=["C2M_SCL"]; f_gt reference
read from gauge_scan/gauge_mismatch_C2M_SCL.csv (falling back to the pooled
gauge_mismatch.csv is unnecessary - SCL rows carry their own s_pose/s_depth).
...
Writes gauge_scan/gauge_gtfree_C2M_SCL.json + gauge_gtfree_curves_C2M_SCL.csv.
...
def main():
    ...
    f_gt = {}
    gm_path = os.path.join(OUT, "gauge_mismatch_C2M_SCL.csv")
    ...
    with open(os.path.join(OUT, "gauge_gtfree_C2M_SCL.json"), "w") as f:
        json.dump(results, f, indent=1)
    with open(os.path.join(OUT, "gauge_gtfree_curves_C2M_SCL.csv"), "w", newline="") as f:
