#!/usr/bin/env python3
"""GT-free gauge-repair factor search for the 7bc286c1b6 random_forced run
(arms A0_baseline, C2M_maskrel @100v). Same objective as
gauge_scannetpp_gtfree.py: conf-weighted cross-frame nearest-neighbour distance
(objective A, argmin) / inlier ratio (objective B, argmax) on the grid
s in [0.90, 1.10] step 0.005, in the predicted frame (no GT, no global scale).
f_gt reference from gauge_7bc/gauge_mismatch.csv. Writes
gauge_7bc/gauge_gtfree.json + gauge_gtfree_curves.csv. Pure CPU. New file; no
