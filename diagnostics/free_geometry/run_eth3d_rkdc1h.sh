...
SKIP_BASELINE=1 python3 "$DG/eval_viewcounts.py" --manifest "$RR/scene_manifest.json" \
    ...
    --arms C2M_RKDC1H --view_subsets allv >> "$LOG" 2>&1
python3 "$DG/run_eval.py" --run_root "$RR" --datas eth3d \
    --experiments "C2M_RKDC1H@allv" >> "$LOG" 2>&1
