#!/bin/bash
# dev3：C2M_RABS1 C2M_RABS5（rel-pose + 绝对平移Huber+FoV，权重1.0）三场景对照
...
out['note'] = 'dev3 C2M_RABS1 C2M_RABS5 (rel + absT/FL w=1.0); entries verbatim from final_protocol/scannetpp'
...
python3 "$DG/train_arms.py" --run_root "$RR" --arms C2M_RABS1 C2M_RABS5 \
...
python3 "$DG/eval_viewcounts.py" --manifest "$RR/scene_manifest.json" \
    ...
    --arms C2M_RABS1 C2M_RABS5 --view_subsets 100v >> "$LOG" 2>&1

echo "=== metrics $(date '+%F %T') ===" >> "$LOG"
python3 "$DG/run_eval.py" --run_root "$RR" --datas scannetpp \
    --experiments "A0_baseline@100v" "C2M_RABS1 C2M_RABS5@100v" >> "$LOG" 2>&1
mv "$RR/eval32_metrics.json" "$RR/eval32_metrics_RABS_dev3.json"
