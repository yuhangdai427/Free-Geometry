...
SUM=workspace/overnight/u300_SUMMARY.log
pat() { echo "[$(date '+%F %T')] $*" >> $SUM; }
SCENES=$($PY -c "import json;print(' '.join(sorted(json.load(open('$MAN'))['scenes'])))")
for SC in $SCENES; do
  OUT=workspace/overnight/da3_dtu_u300/$SC
  pat "DA3-u300 $SC start"
  if $PY scripts/train_pw0_accum.py --dataset dtu --scene $SC --vggt_sync \
      --half_mode vggt --manifest $MAN --updates 300 --accum 1 --out $OUT \
      > logs/dtu_da3_u300_${SC}.log 2>&1; then
    grep "cam_dec" logs/dtu_da3_u300_${SC}.log >> $SUM
  else
    pat "DA3-u300 $SC FAILED"
  fi
