...
SUM=workspace/overnight/u200_SUMMARY.log
pat() { echo "[$(date '+%F %T')] $*" >> $SUM; }
SCENES=$($PY -c "import json;print(' '.join(sorted(json.load(open('$MAN'))['scenes'])))")
for SC in $SCENES; do
  OUT=workspace/overnight/da3_dtu_u200/$SC
  pat "DA3-u200 $SC start"
  if $PY scripts/train_pw0_accum.py --dataset dtu --scene $SC --vggt_sync \
      --half_mode vggt --manifest $MAN --updates 200 --accum 1 --out $OUT \
      > logs/dtu_da3_u200_${SC}.log 2>&1; then
    grep "cam_dec" logs/dtu_da3_u200_${SC}.log >> $SUM
  else
    pat "DA3-u200 $SC FAILED"
  fi
