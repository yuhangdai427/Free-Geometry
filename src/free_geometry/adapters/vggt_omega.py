...
Native VGGT-Omega interface (2026-09-17):
...
class VGGTOmegaAdapter(BaseAdapter):
    ...
    def _full_forward(self, model, images):
        ...
        with torch.autocast(device_type="cuda", enabled=False):
            tokens_list = [x.float() if x is not None else None for x in tokens_list]
            pose_enc = model.camera_head(
                tokens_list, patch_token_start=psi
            )  # joint over ALL frames
            depth, conf = model.dense_head(
                tokens_list, images=images, patch_token_start=psi
            )
        self._ph, self._pw = (
            images.shape[-2] // OMEGA_PATCH,
            images.shape[-1] // OMEGA_PATCH,
        )
    ...
    def forward_teacher(self, images_N, shared_slots):
        model = self.teacher
        with torch.no_grad():
            tokens_list, _psi, pose_enc, depth, conf = self._full_forward(
                model, images_N
            )
            readouts = {
                k: v[:, shared_slots]
                for k, v in self._readouts(model, tokens_list).items()
            }
            out = {
                ...
                "centers": self._centers(
                    model, pose_enc, images_N.shape[-2], images_N.shape[-1]
                )[shared_slots].detach(),
                "ext_w2c": self._ext_w2c(
                    pose_enc, images_N.shape[-2], images_N.shape[-1]
                )[shared_slots].detach(),
            }
        return out
    ...
    def forward_student(self, images_M):
        model = self.model
        tokens_list, _psi, pose_enc, depth, conf = self._full_forward(model, images_M)
        return {
            "readouts": self._readouts(model, tokens_list),
            "depth": depth[0],
            "conf": conf[0],
            "centers": self._centers(
                model, pose_enc, images_M.shape[-2], images_M.shape[-1]
            ),
    ...
    def export_eval(self, images_eval):
        ...
        with torch.no_grad():
            _tokens_list, _psi, pose_enc, depth, conf = self._full_forward(
                model, images_eval
            )
            ext, intr = encoding_to_camera(
                pose_enc.float(), (images_eval.shape[-2], images_eval.shape[-1])
            )
        ext4 = (
            torch.eye(4, device=ext.device)
            .unsqueeze(0)
            .repeat(ext.shape[0], ext.shape[1], 1, 1)
        )
        ext4[:, :, :3, :4] = ext
        return {
            "depth": depth[0].float().cpu(),
            "conf": conf[0].float().cpu(),
            "extr_w2c": ext4[0].float().cpu(),
