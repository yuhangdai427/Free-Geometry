"""DVLT (Deja View Looping Transformer) adapter — FULL fine-tune (no LoRA),
small LR, per plan 2026-09-17 (user decision: DVLT is a 117M model).
...
- eval-mode forward() routes to @torch.no_grad forward_inference; train mode
  ...
  forward_adapt() mirrors forward_inference's body (internals carry no no_grad)
...
class DVLTAdapter(BaseAdapter):
    model_key = "dvlt"
    uses_lora = True  # LoRA r32 on shared recurrent block (user 2026-09-17)
    ...
    def reset_student(self, device="cuda"):
        ...
        self.net.eval()
        self.model = self.net.base_model.model  # underlying DVLTModel
    ...
    def student_params(self):
        m = getattr(self, "net", None) or getattr(self, "model", None) \
            or getattr(self, "teacher", None)
    ...
    def student_label(self):
        ...
        return f"lora_r{self.lora_rank}_{n / 1e6:.1f}M"
    ...
    def _forward_adapt(self, model, images, want_readouts):
        ...
        features = model._solve_inference(x, rope_pos, B, S)  # fixed K=12
