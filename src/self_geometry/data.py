...
def prepare(name, scene, c, directory):
    ...
    data = Evaluator._sample_frames(SimpleNamespace(max_frames=c['max_frames']),ds.get_data(scene),scene)
    files = list(data.image_files)
    manifest = dict(dataset=name,scene=scene,image_files=files,max_frames=c['max_frames'],sampling_seed=42,
...
def _evaluate(name, scene, c, directory, stage, fuse_only=False, score_only=False):
    ...
    for mode in ['recon_unposed','recon_posed']:
        ...
        if not score_only and not fused:
